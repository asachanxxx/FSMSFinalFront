...
import { RfCardMaster, RfCardStatus, CardTypes } from '../../model/RfCardMaster.models';
...
@Component({
  selector: 'app-rfcards',
  templateUrl: './rfcards.component.html',
  styleUrls: ['./rfcards.component.css']
})
export class RfcardsComponent implements OnInit {
  ...
  myform: FormGroup;
  selectedRow: any;
  selectedItem: RfCardMaster;
  holdvar: RfCardMaster[] = [];
  filterholder: RfCardMaster[];
  obj: RfCardMaster = new RfCardMaster();
  ...
  showSuccess(message: string) {
    ...
    this.selectedItem = new RfCardMaster();
  }
  ...
  //************************************************************************** OnInit ***************************************
  ngOnInit() {
    this.CardTypes = RfCardStatus.CardTypes;
  ...
  Filter() {
    ...
    this._http.get<RfCardMaster[]>(this.gloconfig.GetConnection("RfCardMaster", "GetAll"), { headers: this.customHeaders })
      .subscribe(
      data => {
  ...
  onSubmit(myform, event, btn) {
    ...
    if (myform.value.CardNo === "" || myform.value.CardNo === null || myform.value.CardNo === undefined) {
      this.obj.CardNo = " - "
    }
    else {
      this.obj.CardNo = myform.value.CardNo
    }
    if (myform.value.IssuedBy === "" || myform.value.IssuedBy === null || myform.value.IssuedBy === undefined) {
      this.obj.IssuedBy =1
    }
    else {
      this.obj.IssuedBy = myform.value.IssuedBy
    }
    if (myform.value.IssueDate === "" || myform.value.IssueDate === null || myform.value.IssueDate === undefined) {
      this.obj.IssueDate = new Date()
    }
    else {
      this.obj.IssueDate = myform.value.IssueDate
    }
    if (myform.value.CardStatus === "" || myform.value.CardStatus === null || myform.value.CardStatus === undefined) {
      this.obj.CardStatus = 1
    }
    else {
      this.obj.CardStatus = myform.value.CardStatus
    }
  ...
  //************************************************************************** SAVE ***************************************
  Save(item: RfCardMaster) {
    console.log("Save Confirmed!", this.myform.valid);
    this._http.post(this.gloconfig.GetConnection("RfCardMaster", "SaveAsync"), item)
      .subscribe(
      data => {
        console.log(data)
  ...
  //************************************************************************** UPDATE ***************************************
  Update(item: RfCardMaster) {
    this._http.post(this.gloconfig.GetConnection("RfCardMaster", "UpdateAsync"), item)
      .subscribe(
      data => {
        console.log(data)
        if (data === true) {
  ...
  //************************************************************************** DELETE CONFIRM ***************************************
  deleteConfirm(item: RfCardMaster) {
  ...
  Delete(id: number) {
    this._http.post(`${this.gloconfig.GetConnection("RfCardMaster", "DeleteAsync")}?id=${id}`, id)
      .subscribe(
      data => {
        console.log(data)
        if (data === true) {
  ...
  onSelectCardStatus(val:any){
    console.log("onSelectCardStatus", val);
  }
